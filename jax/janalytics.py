...
# eq0_V_args = list(eq0_V.free_symbols) # [V_vv, V_uu, V, j, C, v, a, u]
# eq0_Fu_args = list(eq0_Fu.free_symbols) # [C, n, u, Fv_v, Fv_uv, v, Fu_vv, Fv_u, a, Fu]
# eq0_Fv_args = list(eq0_Fv.free_symbols) # [v, Fu_u, Fu_v, Fv_uu, C, Fv, a, Fu_uv, u]
# eq0_C_args = list(eq0_C.free_symbols) # [v, Fu, C_uu, n, V, C, C_vv, Fv, a, u]
# Eu_args = list(Eu.free_symbols) # [u, v, a, V_u]
# Ev_args = list(Ev.free_symbols) # [V_v, v, a, u]
# B_args = list(B.free_symbols) # [v, Fv_u, Fu_v, Fu, Fv, a, u]
...
if __name__ == '__main__':
    print('Unit test for jax analytics:')
    import sys 
    sys.path.append('../')
    import numpy as np 
    import time
    ...
    @jit
    def get_lambdified(a1, a2, a3, a4, a5, a6, a7):                      
        b1 = eq0_V_lambdified(*a1)
        b2 = eq0_Fu_lambdified(*a2)
        b3 = eq0_Fv_lambdified(*a3)
        b4 = eq0_C_lambdified(*a4)
        b5 = Eu_lambdified(*a5)
        b6 = Ev_lambdified(*a6)
        b7 = B_lambdified(*a7)
    ...
    def lambdified_test(output=True):
        a1, a2, a3, a4, a5, a6, a7 = (jnp.array(np.random.rand(8)), jnp.array(np.random.rand(10)), jnp.array(np.random.rand(9)),
                                      jnp.array(np.random.rand(10)), jnp.array(np.random.rand(4)), jnp.array(np.random.rand(4)),
                                      jnp.array(np.random.rand(7)))
        
        t = time.time()
        get_lambdified(a1, a2, a3, a4, a5, a6, a7)
        if output: print('Time to jax:', time.time() - t)

    lambdified_test(False)
    lambdified_test(True)
